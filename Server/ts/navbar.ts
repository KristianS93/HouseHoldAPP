...
function addAlert(alertLevel: string, alertMessage: string, target: string) {
    let alert = 
    `
    <div class="container">
...
async function register() {
    ...
    let response = await fetch("http://localhost:8888/register", 
    {
        ...
        body: JSON.stringify({
            FirstName: (<HTMLInputElement>document.getElementById(Elements.RegisterFirstName)).value,
            LastName: (<HTMLInputElement>document.getElementById(Elements.RegisterLastName)).value,
            UserID: (<HTMLInputElement>document.getElementById(Elements.RegisterEmail)).value,
            Password: (<HTMLInputElement>document.getElementById(Elements.RegisterPassword)).value,
            ConfirmPassword: (<HTMLInputElement>document.getElementById(Elements.RegisterPasswordConfirm)).value
        })
    })
...
function checkRegistration() {
    ...
    if ((<HTMLInputElement>document.getElementById(Elements.RegisterPassword)).value != (<HTMLInputElement>document.getElementById(Elements.RegisterPasswordConfirm)).value) {
...
function alertParagraph(message: string) {
...
async function login() {
    ...
    let response = await fetch("http://localhost:8888/login", 
    {
        ...
        body: JSON.stringify({
            UserID: (<HTMLInputElement>document.getElementById(Elements.LoginEmail)).value,
            Password: (<HTMLInputElement>document.getElementById(Elements.LoginPassword)).value
        })
    })
...
function validEmail(targetID: string) {
    let email = (<HTMLInputElement>document.getElementById(targetID)).value
...
// only checkcs length
function validPassword(targetID:string ) {
    let password = (<HTMLInputElement>document.getElementById(targetID)).value
...
// checks non empty
function validName(target: string) {
    let name = (<HTMLInputElement>document.getElementById(target)).value
    if (name == "") {
        return false
    }
    return true
